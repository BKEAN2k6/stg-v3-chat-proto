...
import {Comment, Reaction, CommentReactionNotification} from '../../../models';
import {
...
export async function createCommentReaction(
  ...
  const communityId = comment.community._id.toHexString();
  ...
  response
    .status(201)
    .json(reaction.toJSON() satisfies CreateCommentReactionResponse);
