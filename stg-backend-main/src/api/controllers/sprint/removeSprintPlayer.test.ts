...
const removeSprintPlayer = applySchemas(
  routes['/sprints/:id/players/:playerId'].delete,
);
...
describe('removeSprintPlayer', () => {
  ...
  describe('when sprint or player is not found', () => {
    ...
    beforeEach(async () => {
      ...
      await removeSprintPlayer(mocks.req, mocks.res);
      await mocks.result;
    });
    ...
    it('sends an error response', async () => {
      ...
      expect(mocks.res._getJSONData()).toEqual({
        error: 'Sprint or player not found',
      });
    });
  });
  ...
  describe('when player is successfully removed', () => {
    ...
    it('emits the updated player list', async () => {
      expect(mocks.req.events.emit).toHaveBeenCalledWith(
        `/sprints/${mocks.req.params.id}/player`,
        'patch',
        {
          players: [],
        },
