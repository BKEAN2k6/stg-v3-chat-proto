...
There are individual buckets for each environment (currently only production).

The buckets are named like: `stg-backend-[ENVIRONMENT]`

The bucket access is isolated by having one IAM user per bucket (only has access the contents of that bucket)

The users are named like: `stg-backend-[ENVIRONMENT]-s3-user`

The only policy a user is given is one that gives access to an individual bucket (see below for the JSON).

The policies are named like: `full-access-to-stg-backend-[ENVIRONMENT]-s3-bucket`
...
- Move on and give the policy a corret name based on environment (see above for expected policy name)
    ...
    "Statement": [
        {
            ...
            "Effect": "Allow",
            "Resource": [
                "arn:aws:s3:::stg-backend-[ENVIRONMENT]/*"
            ]
        }
    ]
}
...
- In the `Set permissions` screen, select only the policy that matches the user based on the environment (`Attach policies directly`)
    ...
    "Statement": [
        {
            ...
            "Effect": "Allow",
            "Principal": {
                "AWS": "arn:aws:iam::643000180729:user/stg-backend-[ENVIRONMENT]-s3-user"
            },
            "Action": [
                "s3:GetBucketLocation",
                "s3:ListBucket"
            ],
            "Resource": "arn:aws:s3:::stg-backend-[ENVIRONMENT]"
        },
        {
            "Sid": "",
            "Effect": "Allow",
