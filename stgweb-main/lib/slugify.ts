...
const harmonize = (
  ...
  if (ignoreInvalid) {
    return harmonized.replaceAll(/\s+/g, delimiter);
  }

  return harmonized
    .replaceAll(new RegExp(`[^a-z0-9${safeDelimiter}]+`, 'g'), delimiter) // Replace all non-valid caracters by delimiter
    .replaceAll(new RegExp(`${safeDelimiter}+`, 'g'), delimiter) // Remove multiple delimiters repetition
    .replaceAll(new RegExp(`^${safeDelimiter}`, 'g'), '') // remove delimiter at the beginning
    .replaceAll(new RegExp(`${safeDelimiter}$`, 'g'), ''); // remove delimiter at the end
};
  ...
  if (Symbol.iterator in node) {
    return slugify(
      Array.from(node as any)
        .map((subNode) => slugify(subNode as any, {delimiter}))
        .join(delimiter),
      options,
    );
  }
