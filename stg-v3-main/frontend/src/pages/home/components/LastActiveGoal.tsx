...
export default function LastActiveGoal({defaultStrength}: Properties) {
  ...
  const previousModeReference = useRef(fullsScreenGoalMode);
  ...
  useEffect(() => {
    ...
    if (fullsScreenGoalMode !== 'closed') {
      previousModeReference.current = fullsScreenGoalMode;
      return;
    }

    const wasJustClosed = previousModeReference.current !== 'closed';
    previousModeReference.current = fullsScreenGoalMode;
