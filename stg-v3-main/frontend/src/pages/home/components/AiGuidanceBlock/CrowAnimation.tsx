...
export default function CrowAnimation({
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      if (isLoadingRef.current) {
        // Currently loading — start from intro
        setPhase('intro');
        anim.goToAndPlay(introSegment.start, true);
      } else {
        // Data already cached — start directly in idle
        setPhase('idle');
        anim.goToAndPlay(idleSegments[0].start, true);
      }
  ...
  // React to isLoading becoming false, or being false on mount (cached data)
  ...
  useEffect(() => {
    ...
    if (wasLoading && !isLoading) {
      ...
      // Mounted with data already cached — skip loading loop
      pendingTransitionRef.current = 'speaking';
    }
  }, [isLoading]);
