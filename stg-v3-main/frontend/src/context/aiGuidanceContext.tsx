...
export function AiGuidanceProvider({children}: ProviderProperties) {
  ...
  const pendingRequestRef = useRef<
    | {
        ...
        language?: string;
        ageGroup?: string;
        promise: Promise<AiGuidanceData | undefined>;
      }
    | undefined
  >(undefined);
  ...
  const refetch = useCallback(
    async (
      ...
      const promise = (async (): Promise<AiGuidanceData | undefined> => {
        try {
          const now = new Date();
  ...
  const fetchGuidance = useCallback(
    async (
      ...
      // Reuse in-flight request if parameters match
      if (
        pendingRequestRef.current?.groupId === groupId &&
        (!context?.language ||
          pendingRequestRef.current.language === context.language) &&
        (!context?.ageGroup ||
          pendingRequestRef.current.ageGroup === context.ageGroup)
      ) {
