...
import {type CreateCommunityUserImageResponse} from '@client/ApiTypes';
...
type MediaUploadProperties = {
  readonly existingImages?: Array<{id: string; previewUrl: string}>;
  readonly children: ReactNode;
};

function MediaUpload(properties: MediaUploadProperties) {
  const {existingImages, children} = properties;
...
function Trigger(properties: TriggerProperties) {
    ...
    async onDropAccepted(acceptedFiles) {
      ...
      const newImageUploads = acceptedFiles.map(async (file) => {
        try {
          ...
          return {id: uploadResponse.id, previewUrl};
        } catch {
          toasts.danger({
...
function Preview(properties: PreviewProperties) {
  const context = useContext(MediaUploadContext);
  if (!context)
    throw new Error(
      'MediaUpload.Trigger component cannot be rendered outside the MediaUpload component',
