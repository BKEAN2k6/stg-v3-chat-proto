...
export default function ReactionsModal(properties: Properties) {
  const {i18n} = useLingui();
  const {isOpen, onClose, reactions} = properties;
    ...
    <Modal centered show={isOpen} className="px-4" onHide={handleClose}>
      ...
      <Modal.Body className="">
        ...
        {reactions.map((reaction) => (
          <div
            key={`reaction-modal-${reaction.id}`}
          ...
          >
            <Avatar
              ...
              color={colorFromId(reaction.createdBy.id)}
            />
