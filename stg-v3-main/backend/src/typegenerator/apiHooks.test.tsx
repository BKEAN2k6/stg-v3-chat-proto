...
const createTemporaryDirectory = () =>
  fs.mkdtempSync(path.join(tmpdir(), 'api-hooks-'));
...
const importGeneratedHooks = async (): Promise<{
  ...
  // Pass mocks via globalThis so the temp module can access them
  (globalThis as any).__apiHookTestMocks = apiMocks;

  const directory = createTemporaryDirectory();

  // Write mock ApiClient.js and ApiTypes.js alongside the generated hooks
  writeTemporaryFile(
    directory,
    'ApiClient.js',
    `export class ApiError extends Error {}
const api = globalThis.__apiHookTestMocks;
export default api;
`,
  );
  writeTemporaryFile(directory, 'ApiTypes.js', '');

  const fileUrl = writeTemporaryFile(
    directory,
    'hooks.generated.mjs',
