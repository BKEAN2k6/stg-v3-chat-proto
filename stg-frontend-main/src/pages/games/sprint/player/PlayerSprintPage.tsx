...
export default function PlayerSprintPage() {
  ...
  }, [sprint?.isStarted, sprint?.isEnded, fetchSprintData]);
  ...
  const handleStrengthChange = async (strength: StrengthSlug, to: string) => {
    try {
      ...
      await api.createSprintStrength({id: sprint._id}, {strength, to});
  ...
  if (sprint && !sprint.players.includes(sprint.player._id)) {
    navigate('/games/sprints/join');
    return null;
  }
  ...
  let screen = 'pick';
  if (!sprint.isStarted) screen = 'wait';
  if (sprint.isEnded) screen = 'result';
      ...
      return (
        <CenterAligned>
          <div>
            ...
            <p className="mt-5">
              <Trans>Just a moment, waiting for the sprint to start.</Trans>
            </p>
            <div className="mt-5">
              <Loader />
            </div>
            ...
            {selectedPlayerIndex < sprint.room.length && (
              <>
                <span className="mt-3 d-block text-muted text-center">
                  {selectedPlayerIndex + 1} <Trans>of</Trans>{' '}
                  {sprint.room.length}
                </span>
